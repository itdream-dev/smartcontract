...
contract MarketCore is Initializable, PausableUpgradeable, OrderValidator, TransferManager {
    ...
    function matchAndTransfer(LibOrder.Order memory orderLeft, LibOrder.Order memory orderRight) internal {        
        ...
        if (orderLeft.makeAsset.assetType.assetClass == LibAsset.ERC721_ASSET_CLASS) {                               
            ...
            (creator, royaltyPrice) = getRoyaltyInfoERC721(orderLeft.makeAsset, price);    
            if (royaltyPrice > 0)
            ...
            (creator, royaltyPrice) = getRoyaltyInfoERC721Lazy(orderLeft.makeAsset, price);                        
            if (royaltyPrice > 0)
                address(creator).transferEth2(royaltyPrice);
            ...
            (creator, royaltyPrice) = getRoyaltyInfoERC1155(orderLeft.makeAsset, price);                        
            if (royaltyPrice > 0)
            ...
            (creator, royaltyPrice) = getRoyaltyInfoERC1155Lazy(orderLeft.makeAsset, price);                        
            if (royaltyPrice > 0)
                address(creator).transferEth3(royaltyPrice);
        }     
